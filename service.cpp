...
namespace KaizenLibrary
{
   ...
   std::string Services::truncate(char* json) 
   {
	...
	/*try {
		...
		*/
	Value& variance = d["variance"];
	Value& states = d["states"];
	int nStates = states.GetArray().Size();

	RNode* n = network->createNode(0, nStates);

	for (int i = 0; i < states.GetArray().Size(); i++) {
		n->setStateValue(i, states[i].GetDouble());
	}

	/*
	n->setStateValue(0, states[0].GetDouble());
	n->setStateValue(1, states[1].GetDouble());
	n->setStateValue(2, states[2].GetDouble());
	*/

	n->setVariance(variance.GetDouble());
	n->generatePseudoNormalDistribution();
	n->truncateMe();
	
	for (int k = 0; k < states.Size(); ++k) {
		states[k] = n->getStateValuesPtr()->at(k);
	}
